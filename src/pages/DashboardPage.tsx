      ...
      <main className="flex-1 min-h-screen flex flex-col min-w-0 lg:pl-64 transition-all duration-300">
        ...
        <header className="bg-white border-b border-slate-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 sticky top-0 z-30 shadow-2xs">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div
              ...
              className="lg:hidden flex items-center gap-2 cursor-pointer group/logo shrink-0"
              title="Go to Landing Page"
            >
              <div className="w-7 h-7 rounded-xl bg-blue-600 text-white font-extrabold text-xs flex items-center justify-center shadow-xs">
                S
              </div>
        ...
        {activeTab === 'pay' && (
          <div className="p-4 sm:p-6 max-w-3xl mx-auto w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* Left Column: Quick Pay & Current Due Details */}
              <div className="lg:col-span-1 space-y-6">
                
                {/* Modern Due Amount Card */}
                <div className="relative overflow-hidden bg-slate-900 rounded-3xl p-6 shadow-2xl text-white">
                  ...
                  <div className="absolute -bottom-24 -left-24 w-48 h-48 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-pulse" style={{ animationDelay: '2s' }}></div>
                  
                  <div className="relative z-10 flex flex-col h-full justify-between space-y-8">
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <span className="bg-white/10 backdrop-blur-md border border-white/20 text-white text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider">
                          Current Cycle
                        </span>
                    ...
                    <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4">
                      <p className="text-xs text-slate-300 mb-1 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-rose-400" /> Due Date</p>
                ...
                <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wide">Final Payment Date</p>
                    <p className="text-sm font-bold text-slate-900 mt-0.5">
                      {membership ? new Date(new Date(membership.created_at).setMonth(new Date(membership.created_at).getMonth() + plan.duration_months)).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'}
                    </p>
                  </div>
                </div>
              </div>

              {/* Right Column: Interactive Payment History Timeline */}
              <div className="lg:col-span-2">
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
                  <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div>
                      <h3 className="font-bold text-lg text-slate-900">Payment History</h3>
                      <p className="text-xs text-slate-500">Track all your monthly deposits for this plan.</p>
                    </div>
                    <span className="bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1.5 rounded-full border border-blue-100">
                      {contributions.length} / {plan.duration_months} Paid
                    </span>
                  </div>
                  ...
                  <div className="flex-1 p-6 space-y-4 max-h-[600px] overflow-y-auto bg-slate-50 overscroll-contain">
                    ...
                    {contributions.map((c, index) => (
                      <div 
                        key={c.id} 
                        className="group flex items-center justify-between p-4 sm:p-5 bg-white rounded-2xl border border-slate-200 hover:border-emerald-300 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5"
                      >
                        <div className="flex items-center gap-4 sm:gap-6">
                          <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center font-bold text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors">
                            {c.cycle_number}
                          </div>
                          <div>
                            <p className="font-bold text-sm text-slate-900">Month #{c.cycle_number} Deposit</p>
                            <p className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                              <Calendar className="w-3 h-3" /> Due: {new Date(c.due_date).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
                    ...
                    {/* Render Upcoming Pending Contributions */}
                    {Array.from({ length: plan.duration_months - contributions.length }).map((_, i) => {
                      const upcomingCycle = contributions.length + i + 1;
                      const isNext = i === 0;
                      return (
                        <div 
                          key={`upcoming-${i}`} 
                          className={`flex items-center justify-between p-4 sm:p-5 bg-white/60 rounded-2xl border border-slate-200 border-dashed ${isNext ? 'border-blue-300 bg-blue-50/30' : 'opacity-70'}`}
                        >
                          <div className="flex items-center gap-4 sm:gap-6">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-slate-400 ${isNext ? 'bg-blue-100 text-blue-600' : 'bg-slate-100'}`}>
                              {upcomingCycle}
                            </div>
                            <div>
                              <p className="font-bold text-sm text-slate-900">Month #{upcomingCycle} Deposit</p>
                              <p className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                                <Clock className="w-3 h-3" /> {isNext ? 'Next Due' : 'Upcoming'}
                              </p>
                            </div>
                          </div>
