...
`;
  ...
  // Admin Navigation Options Requested
  const navItems: Array<{ id: string; label: string; icon: any; badge?: string; disabled?: boolean }> = [
  ...
  return (
    <div className="bg-[#F4F6F9] flex min-h-screen w-full text-slate-800 font-sans relative">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 h-[100dvh] ${
          isSidebarCollapsed ? 'w-20' : 'w-72 max-w-[85vw] lg:w-64'
        } bg-[#1E2640] text-slate-300 flex flex-col shrink-0 border-r border-slate-800 transition-all duration-300 ease-in-out overscroll-contain touch-auto ${
      ...
      <div className={`flex-1 min-h-screen flex flex-col min-w-0 ${isSidebarCollapsed ? 'lg:pl-20' : 'lg:pl-64'} transition-all duration-300`}>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6 animate-in fade-in duration-200">
              ...
              {/* Enclosed 6-Stage Customer Lifecycle Pipeline Box */}
              <div className="bg-white p-6 rounded-3xl border border-[#E8EAF8] shadow-sm space-y-4 min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#E8EAF8] pb-3">
                  <div>
                    <h3 className="font-heading font-extrabold text-xl text-[#1F1F24]">6-Stage Customer Lifecycle Pipeline</h3>
                    <p className="text-xs text-[#6C7285] mt-0.5">Enclosed customer stage tracking board with internal column scrolling</p>
                  </div>
                  <span className="text-xs font-bold text-[#4F5DFF] bg-blue-50 px-3 py-1 rounded-full border border-blue-100 self-start sm:self-auto">
                    {profiles.filter(p => p.role === 'member').length} Total Members
                  </span>
                </div>

                <div className="bg-[#F8FAFC] p-3.5 rounded-2xl border border-[#E2E8F0] overflow-x-auto scrollbar-thin">
                  <div className="flex gap-4 min-w-max">
                    {lifecycleStages.map((stage) => {
                      ...
                      return (
                        <div
                          key={stage.id}
                          className="w-[270px] bg-white rounded-2xl border border-[#E2E8F0] shadow-2xs flex flex-col max-h-[500px] overflow-hidden shrink-0"
                        >
                          {/* Sticky Stage Header */}
                          <div className="p-3 bg-[#F7F8FC] border-b border-[#E2E8F0] flex items-center justify-between sticky top-0 z-10 shrink-0">
                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${stage.badgeStyle}`}>
                              {stage.name}
                            </span>
                            <span className="font-mono font-bold text-xs bg-slate-200 text-slate-700 px-2 py-0.5 rounded-md">
                              {stageMembers.length}
                            </span>
                          </div>

                          {/* Vertically Scrollable Member Cards Container */}
                          <div className="p-3 overflow-y-auto scrollbar-thin space-y-2.5 flex-1 max-h-[440px]">
                            {stageMembers.length === 0 ? (
                              <div className="h-32 flex items-center justify-center border-2 border-dashed border-slate-200 rounded-xl p-4 text-[11px] text-slate-400 font-medium text-center">
                                No Members
                              </div>
                            ) : (
                              stageMembers.map((m: any, idx: number) => (
                                <div
                                  key={`${m.id}-${idx}`}
                                  className="bg-white p-3 rounded-xl border border-[#E8EAF8] shadow-2xs hover:shadow-md hover:border-[#4F5DFF]/40 transition-all space-y-2 group/card"
                                >
                                  <div className="flex items-center gap-2.5">
                                    <div className="w-8 h-8 rounded-full bg-[#4F5DFF] text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-xs group-hover/card:scale-105 transition-transform">
                                      {m.full_name?.charAt(0) || 'M'}
                                    </div>
                                    <div className="overflow-hidden flex-1">
                                      <p className="font-bold text-xs text-[#1F1F24] truncate group-hover/card:text-[#4F5DFF] transition-colors">
                                        {m.full_name}
                                      </p>
                                  ...
                                  <div className="pt-1 flex items-center gap-1.5 border-t border-[#E8EAF8]">
                                    <button
                                      onClick={() => setInspectedMember(m)}
                                      className="flex-1 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold text-[10px] py-1.5 rounded-lg border border-slate-200 transition-all text-center cursor-pointer"
                                    >
                                      Inspect 360°
                                    </button>
                                    {(m.kyc_status !== 'approved' || stage.id === 'pending') && (
                                      <button
                                        onClick={async () => {
                                          await stateStore.updateKYCStatus(m.id, 'approved', adminUser.id);
                                          setProfiles([...stateStore.getProfiles()]);
                                        }}
                                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] py-1.5 rounded-lg shadow-2xs transition-all text-center cursor-pointer active:scale-95"
                                      >
                                        Approve
                                      </button>
                                    )}
