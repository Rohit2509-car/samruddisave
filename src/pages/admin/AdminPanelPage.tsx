...
export const AdminPanelPage: React.FC<AdminPanelPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 space-y-8">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-900 via-rose-950 to-slate-900 text-white p-7 sm:p-9 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border border-slate-800">
        <div>
          <span className="bg-rose-500/20 text-rose-300 border border-rose-400/30 text-xs font-bold px-3.5 py-1 rounded-full uppercase tracking-wider">
        ...
        <button
          ...
          className="bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 px-6 rounded-full transition-all shadow-md text-xs flex items-center gap-2 shrink-0"
        >
      ...
      <div className="nexora-card p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
        <h3 className="font-heading font-bold text-lg text-slate-900">Platform Staff Roster & Department Roles</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
          {profiles.filter(p => p.role !== 'member').map(staff => (
            <div key={staff.id} className="p-4 bg-slate-50/80 border border-slate-200/70 rounded-2xl space-y-1">
              <span className="bg-purple-100 text-purple-900 text-[9px] font-bold px-2.5 py-0.5 rounded-full uppercase">
                {staff.role}
              </span>
              <p className="font-bold text-slate-900 text-sm mt-1.5">{staff.full_name}</p>
      ...
      <div className="nexora-card p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
          <div>
            ...
            <h3 className="font-heading font-extrabold text-2xl sm:text-3xl text-slate-900 mt-1.5">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200/80 focus:border-blue-600 rounded-xl pl-9 pr-3.5 py-2 text-xs text-slate-900 outline-none"
            />
          </div>
        </div>
        ...
        <div className="space-y-3 font-mono text-xs max-h-96 overflow-y-auto pr-1">
          {filteredLogs.map((log) => (
            <div key={log.id} className="bg-slate-900 text-slate-200 p-4 rounded-2xl space-y-1.5 border border-slate-800">
              ...
              <p className="text-white text-xs font-medium">{log.details}</p>
